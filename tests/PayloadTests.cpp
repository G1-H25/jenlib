...
void test_payload_append_within_capacity(void) {
    jenlib::ble::BlePayload p;
    p.clear();
    for (std::size_t i = 0; i < jenlib::ble::kMaxPayload; ++i) {
...
void test_payload_append_raw_bounds(void) {
    ...
    const std::size_t remaining = jenlib::ble::kMaxPayload - 10;
    TEST_ASSERT_TRUE(p.append_raw(buf, remaining));
    TEST_ASSERT_EQUAL_UINT32(jenlib::ble::kMaxPayload, p.size);
...
void test_payload_read_bounds_and_progress(void) {
    ...
    TEST_ASSERT_TRUE(jenlib::ble::read_u8(p, i, b));
    TEST_ASSERT_EQUAL_UINT8(0x11, b);
    TEST_ASSERT_EQUAL_UINT32(1, i);

    TEST_ASSERT_TRUE(jenlib::ble::read_u16le(p, i, u16));
    TEST_ASSERT_EQUAL_UINT16(0x2233, u16);
    TEST_ASSERT_EQUAL_UINT32(3, i);

    TEST_ASSERT_TRUE(jenlib::ble::read_u32le(p, i, u32));
    ...
    std::uint8_t dummy = 0;
    TEST_ASSERT_FALSE(jenlib::ble::read_u8(p, i, dummy));
    TEST_ASSERT_EQUAL_UINT32(p.size, i);
}
